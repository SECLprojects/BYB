...
create index if not exists event_requests_submitted_at_idx on event_requests (submitted_at desc);
...
create or replace function upsert_contact(p_email text, p_name text, p_organisation text)
returns void as $$
begin
  insert into contacts (email, name, organisation, first_seen_at, last_seen_at, request_count)
